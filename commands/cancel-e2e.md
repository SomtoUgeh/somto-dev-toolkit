...
allowed-tools: ["Bash(test -f .claude/e2e-loop.local.md:*)", "Bash(rm .claude/e2e-loop.local.md)", "Read(.claude/e2e-loop.local.md)", "Bash(echo *>>*)"]
...
3. **If EXISTS**:
   - Read `.claude/e2e-loop.local.md` to get: `iteration` and `progress_path`

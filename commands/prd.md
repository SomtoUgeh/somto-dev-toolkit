...
| 3 | Spec Write | `<phase_complete phase="3" spec_path="..."/>` | 3.5 |
...
Spawn 4 reviewers IN PARALLEL, add findings to spec. Output `<gate_decision>PROCEED</gate_decision>` or `BLOCK`.
...
Spawn prd-complexity-estimator agent, then output `<max_iterations>N</max_iterations>`.
